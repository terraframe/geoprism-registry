...
//    TestDataSet.populateAdapterIds(null, testData.adapter);
  ...
  private void testDeleteDocumentAsUser(ClientRequestIF request)
  {
    ...
    String sJson = service.uploadFileAction(request.getSessionId(), this.actionOid, "parent-test.xlsx", ChangeRequestDocumentActionServiceTest.class.getResourceAsStream("/parent-test.xlsx"));
    
    JsonObject jsonVF = JsonParser.parseString(sJson).getAsJsonObject();
    
    final String vfOid = jsonVF.get("oid").getAsString();
    
    service.deleteDocumentAction(request.getSessionId(), this.actionOid, vfOid);
  ...
  public void testListDocumentsAsUser(ClientRequestIF request)
  {
    ...
    JsonArray ja = JsonParser.parseString(service.listDocumentsAction(request.getSessionId(), this.actionOid)).getAsJsonArray();
  ...
  private String uploadDocumentsAsAdmin()
  {
    ...
    for (int i = 0; i < 2; ++i)
    {
      json = new ChangeRequestService().uploadFileActionInTransaction(this.actionOid, "parent-test.xlsx", ChangeRequestDocumentActionServiceTest.class.getResourceAsStream("/parent-test.xlsx"));
    }
  ...
  public void testUploadDocument()
  {
    ...
    for (TestUserInfo user : allowedUsers)
    {
      ...
      {
        FastTestDataset.runAsUser(user, (request, adapter) -> {
          new ChangeRequestService().uploadFileAction(request.getSessionId(), this.actionOid, "parent-test.xlsx", ChangeRequestDocumentActionServiceTest.class.getResourceAsStream("/parent-test.xlsx"));
  ...
  private void downloadDocumentAsUser(ClientRequestIF request, String vfOid)
  {
    ...
    try (ApplicationResource res = service.downloadDocumentAction(request.getSessionId(), this.actionOid, vfOid))
    {
