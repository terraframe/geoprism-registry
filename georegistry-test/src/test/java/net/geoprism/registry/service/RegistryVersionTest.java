...
public class RegistryVersionTest extends FastDatasetTest implements InstanceTestClassListener
{
  ...
  /**
   * Tests to make sure if we set a value which is encompassed by a larger
   * surrounding date range that the operation is ignored.
   */
  @Test
  @Request
  ...
  //
  // testData.adapter.setAttributeVersions(testData.PROV_CENTRAL.getCode(),
  // testData.PROV_CENTRAL.getGeoObjectType().getCode(), "status", dto);
  //
  // assertVersionData(ValueOverTimeConverter.colToDTO(serverObj.getValuesOverTime(attributeName),
  // type));
  // }
  //
  ...
  // Assert.assertEquals(dto.get(0).getStartDate(),
  // dateFormat.parse("01-01-1990"));
  // Assert.assertEquals(dto.get(0).getEndDate(),
  // dateFormat.parse("01-31-1990"));
  //
  // Assert.assertEquals(dto.get(1).getValue(), GeoObjectStatus.NEW);
  // Assert.assertEquals(dto.get(1).getStartDate(),
  // dateFormat.parse("02-01-1990"));
  // Assert.assertEquals(dto.get(1).getEndDate(),
  // dateFormat.parse("02-28-1990"));
  //
  // Assert.assertEquals(dto.get(2).getValue(), GeoObjectStatus.PENDING);
  // Assert.assertEquals(dto.get(2).getStartDate(),
  // dateFormat.parse("03-01-1990"));
  // Assert.assertEquals(dto.get(2).getEndDate(),
  // ValueOverTimeDTO.INFINITY_END_DATE);
  // }
  ...
  // try
  // {
  // dto.add(new ValueOverTimeDTO(dateFormat.parse("01-01-1990"), null,
  // GeoObjectStatus.INACTIVE, type));
  // dto.add(new ValueOverTimeDTO(dateFormat.parse("02-01-1990"), null,
  // GeoObjectStatus.NEW, type));
  // dto.add(new ValueOverTimeDTO(dateFormat.parse("03-01-1990"), null,
  // GeoObjectStatus.PENDING, type));
  // }
  // catch (ParseException e)
  // {
  // throw new RuntimeException(e);
  ...
  // {
  // return
  // testData.PROV_CENTRAL.getGeoObjectType().getGeoObjectType().getAttribute(attributeName).get();
  // }
}
