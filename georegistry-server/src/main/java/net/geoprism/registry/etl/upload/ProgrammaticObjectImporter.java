...
public class ProgrammaticObjectImporter implements ObjectImporterIF
{
  ...
  private static final Logger                     logger                     = LoggerFactory.getLogger(ProgrammaticObjectImporter.class);

  protected static final String                   ERROR_OBJECT_TYPE          = GeoObjectOverTime.class.getName();

  protected ProgrammaticObjectImportConfiguration configuration;
  ...
  public ProgrammaticObjectImporter(ProgrammaticObjectImportConfiguration configuration, ImportProgressListenerIF progressListener)
  {
  ...
  protected class GeoObjectErrorBuilder
  {
    ...
    protected ProgrammaticObject object;
    ...
    public ProgrammaticObject getObject()
    {
      return object;
    }

    public void setObject(ProgrammaticObject object)
    {
      this.object = object;
    }
    ...
    public JSONArray build()
    {
      ...
      try
      {
        if (this.getGeoObject() != null)
        {
          ...
          final List<Location> locations = ProgrammaticObjectImporter.this.getConfiguration().getLocations();
          final ServerHierarchyType hierarchy = ProgrammaticObjectImporter.this.configuration.getHierarchy();
          ...
          ServerParentTreeNode tnParent = new ServerParentTreeNode(geoObject, hierarchy, ProgrammaticObjectImporter.this.getConfiguration().getDate(), ProgrammaticObjectImporter.this.getConfiguration().getDate(), null);
  ...
  @Override
  public ProgrammaticObjectImportConfiguration getConfiguration()
  {
  ...
  public void validateRow(FeatureRow row) throws InterruptedException
  {
    try
    {
      ...
      if ( ( this.lastValidateSessionRefresh + ProgrammaticObjectImporter.refreshSessionRecordCount ) < curWorkProgress)
      {
      ...
      try
      {
        ...
        ProgrammaticObject entity;

        entity = ProgrammaticObject.newInstance(this.configuration.getType());
  ...
  public void importRow(FeatureRow row) throws InterruptedException
  {
    ...
    catch (ProgrammaticObjectRecordedErrorException e)
    {
      this.recordError(e);
    }
  ...
  @Transaction
  private void recordError(ProgrammaticObjectRecordedErrorException e)
  {
  ...
  public void importRowInTrans(FeatureRow row, RowData data)
  {
    // Refresh the session because it might expire on long imports
    final long curWorkProgress = this.progressListener.getWorkProgress();
    if ( ( this.lastImportSessionRefresh + ProgrammaticObjectImporter.refreshSessionRecordCount ) < curWorkProgress)
    {
      SessionFacade.renewSession(Session.getCurrentSession().getOid());
      this.lastImportSessionRefresh = curWorkProgress;
    }

    ProgrammaticObject programmaticObject = null;
...
//      }
      ...
      {
        // programmaticObject = service.getGeoObjectByCode(code,
        // this.configuration.getType());
      }

      if (programmaticObject == null)
      {
        ...
        programmaticObject = ProgrammaticObject.newInstance(this.configuration.getType());
      }

      builder.setObject(programmaticObject);
      ...
      for (Entry<String, AttributeType> entry : entries)
      {
        ...
        if (function != null)
        {
          ...
          if (value != null && !this.isEmptyString(value))
          {
            this.setValue(programmaticObject, attributeType, attributeName, value);
          }
          else if (this.configuration.getCopyBlank())
          {
            this.setValue(programmaticObject, attributeType, attributeName, null);
          }
        }
        ...
        data.setGoJson(programmaticObject.toJSON().toString());
        data.setNew(isNew);
        data.setParentBuilder(builder);
        
        programmaticObject.setGeoObject(geoObject);
        programmaticObject.apply();
      }
    ...
    {
      buildRecordException(programmaticObject.toJSON().toString(), isNew, builder, t);
    }
  ...
  private void buildRecordException(String goJson, boolean isNew, GeoObjectErrorBuilder parentBuilder, Throwable t)
  {
    ...
    ProgrammaticObjectRecordedErrorException re = new ProgrammaticObjectRecordedErrorException();
    re.setError(t);
...
//          }
  ...
  protected void setTermValue(ProgrammaticObject entity, AttributeType attributeType, String attributeName, Object value)
  {
    if (!this.configuration.isExclusion(attributeName, value.toString()))
    {
      try
      {
        ProgrammaticType type = this.configuration.getType();
  ...
  protected void setClassificationValue(ProgrammaticObject entity, AttributeType attributeType, String attributeName, Object value)
  {
    if (!this.configuration.isExclusion(attributeName, value.toString()))
    {
      try
  ...
  protected void setValue(ProgrammaticObject entity, AttributeType attributeType, String attributeName, Object value)
  {
