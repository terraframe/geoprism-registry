...
//  private SyncLevel syncLevel;
  ...
  protected JsonElement exportObject(GeoObject go) throws IOException
  {
...
//    }
  ...
  /*
   * This code is necessary if doing a DHIS2 export. But it's not used anymore because the DHIS2 exporter does it's own serialization now.
   * 
  ...
  public JsonObject export() throws IOException
  {
    ...
    try (OIterator<GeoObject> it = postgresQuery())
    {
      while (it.hasNext())
      {
        ...
        results.add(this.exportObject(go));
      }
    }
    ...
    page.addProperty("total", this.total);
    jo.add("page", page);
