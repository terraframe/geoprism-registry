...
public class VertexExternalIdRestriction extends AbstractVertexRestriction implements VertexGeoObjectRestriction
{
  ...
  public VertexExternalIdRestriction(ServerGeoObjectType type, SourceAuthority authority, String externalId)
  {
    this.type = type;
  ...
  public void restrict(StringBuilder statement, Map<String, Object> parameters)
  {
    ...
    statement.append("  SELECT expand(outE('" + mdEdge.getDBClassName() + "')[id = :id]) FROM :authority");
    statement.append("))");
    statement.append("WHERE @class = :class ");

    parameters.put("authority", this.authority.getRID());
