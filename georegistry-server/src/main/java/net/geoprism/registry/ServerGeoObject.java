...
public class ServerGeoObject
{
  private Logger logger = LoggerFactory.getLogger(ServerGeoObject.class);
  
  private GeoObject go;
  
  private ServerGeoObject(GeoObject go)
  {
    this.go = go;
  }
  
  public static ServerGeoObject getFromGeoObject(GeoObject go)
  {
    return new ServerGeoObject(go);
  }
  ...
  public Business getLeafBusiness()
  {
  ...
  public String bbox()
  {
    ...
    else
    {
      ...
      try
      {
        ...
        sql.append(" WHERE " + GeoEntity.OID + " = '" + this.getLeafBusiness().getOid() + "'");
