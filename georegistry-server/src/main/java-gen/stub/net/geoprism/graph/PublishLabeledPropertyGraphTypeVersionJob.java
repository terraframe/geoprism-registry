...
public class PublishLabeledPropertyGraphTypeVersionJob extends PublishLabeledPropertyGraphTypeVersionJobBase implements JsonSerializable
{
  ...
  public void execute(ExecutionContext executionContext) throws Throwable
  {
    ...
    dto.setFileName("Publish LPG " + this.getVersion().getGraphType().getDisplayLabel().getValue() + " (Version " + this.getVersion().getVersionNumber() + ")");
    dto.setObjectType(JobHistoryType.LPG);
  ...
  /*
   * It was decided to use the ImportHistory here primarily because of the
   * complicated 'getActiveImports' query in ETLBusinessService, which shows
   * jobs on the 'ScheduledJobs' page. Since this query brings in all sorts of
   * complex "Organization" related criteria, it was decided to use
   * ImportHistory in order to not rock the boat, even though semantically this
   * is not an import.
   */
  @Override
  ...
  //
  // if (history.getStatus().get(0).equals(AllJobStatus.FAILURE) &&
  // history.getErrorJson().length() > 0)
  // {
  // JsonObject exception = new JsonObject();
  //
  // exception.add("type",
  // JsonParser.parseString(history.getErrorJson()).getAsJsonObject().get("type"));
  // exception.addProperty("message",
  // history.getLocalizedError(Session.getCurrentLocale()));
  //
