...
{
  public static enum JobHistoryType {
    GEO_OBJECT, BUSINESS_OBJECT, CONCEPT_OBJECT, EDGE_OBJECT, LPG, RDF_LPG, RDF_REPO
  }

  public static ObjectImporterIF getImporter(String type, ImportConfiguration config, ImportProgressListenerIF progress)
  {
    if (type.equals(JobHistoryType.GEO_OBJECT.name()))
    {
      return new GeoObjectImporter((GeoObjectImportConfiguration) config, progress);
    }
    else if (type.equals(JobHistoryType.BUSINESS_OBJECT.name()))
    {
      return new BusinessObjectImporter((BusinessObjectImportConfiguration) config, progress);
    }
    else if (type.equals(JobHistoryType.CONCEPT_OBJECT.name()))
    {
      return new ConceptObjectImporter((ConceptObjectImportConfiguration) config, progress);
    }
    else if (type.equals(JobHistoryType.EDGE_OBJECT.name()))
    {
